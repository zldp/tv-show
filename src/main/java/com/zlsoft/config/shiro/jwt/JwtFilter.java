...
import com.zlsoft.util.common.JsonConvertUtil;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        // 拿到当前Header中Authorization的AccessToken(Shiro中getAuthzHeader方法已经实现)
        String token = this.getAuthzHeader(request);
        System.out.println("jwt的token"+token);
        // 获取当前Token的帐号信息
        ...
        if (JedisUtil.exists(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            ...
            if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                token = JwtUtil.sign(account, currentTimeMillisRedis);
    ...
    private void response401(ServletRequest req, ServletResponse resp, String msg) {
        ...
        httpServletResponse.setStatus(HttpStatus.OK.value()); // token过期 状态200 （之前是401  前端需要）
        httpServletResponse.setCharacterEncoding("UTF-8");
        httpServletResponse.setContentType("application/json; charset=utf-8");
        PrintWriter out = null;
        try {
            out = httpServletResponse.getWriter();
            String data = JsonConvertUtil.objectToJson(new Result(HttpStatus.UNAUTHORIZED.value(), "无权访问(Unauthorized):" + msg, 401));
            out.append(data);
        } catch (IOException e) {
